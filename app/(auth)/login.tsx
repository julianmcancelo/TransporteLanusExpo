...
type AnimatedShapeProps = { style: any; delay: number };
type ThemeColors = typeof Colors.light & { primary: string, primaryDark: string, primaryLight: string, accent: string, cardBackground: string, grayMedium: string, success: string, warning: string, error: string, info: string, card: string };
type AnimatedHeaderBackgroundProps = { themeColors: ThemeColors };
type ContactModalProps = { visible: boolean; onClose: () => void; themeColors: ThemeColors };
type InputWithIconProps = { icon: ReactNode; themeColors: ThemeColors } & React.ComponentProps<typeof TextInput>;
...
const ActionCard = React.memo(({ icon, title, subtitle, onPress, delay, disabled, themeColors }: ActionCardProps) => {
    const styles = getStyles(themeColors);
...
const LoginScreen = (): React.ReactElement => {
    ...
    const themeColors: ThemeColors = AppColors[colorScheme];
    const styles = getStyles(themeColors);
    ...
    return (
        <View style={styles.container}>
            {renderChoiceView()}
            <Animated.View style={[StyleSheet.absoluteFill, formAnimStyle]} pointerEvents={viewMode === 'manual' || viewMode === 'internal' ? 'auto' : 'none'}>
